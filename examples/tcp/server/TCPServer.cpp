	...
	switch(tcp_step) {
		...
		case 1:
			...
			if (TcpOpen_0->status == ERR_OK) {
				...
				tcp_step = 0;
				EvLog->createEvent(EventLogger::ARELS_ERROR, 0, 0x3000);
				this->_status = ERROR;
			}
			break;
		case 2:
			...
			if (TcpIoctl_0->status == ERR_OK) {
				...
				tcp_step = 0;
				EvLog->createEvent(EventLogger::ARELS_ERROR, 0, 0x3001);
				this->_status = ERROR;
			}
			break;
		case 3:
			...
			if (TcpServer_0->status == ERR_OK) {
				...
				tcp_step = 0;
				EvLog->createEvent(EventLogger::ARELS_ERROR, 0, 0x3002);
				this->_status = ERROR;
			}
			break;
		case 4:
...
TCPServer::Status TCPServer::read()
{
	...
	if (TcpRecv_0->status == ERR_OK)
	{
		...
		if (this->recvlength == 0)
		{
			...
			EvLog->createEvent(EventLogger::ARELS_WARNING, 0, 0x3004);
			return STOP;
		}
		else
			return READY;
	...
	{
		EvLog->createEvent(EventLogger::ARELS_ERROR, 0, 0x3003);
		return ERROR;
	}
	return BUSY;
}
...
void TCPServer::close_sockets()
{
	...
	switch (tcp_step)
	{
		case 0:
			...
			{
				tcp_step = 255;
				EvLog->createEvent(EventLogger::ARELS_ERROR, 0, 0x3005);
				_status = ERROR;
			}
			break;
		case 1:
			...
			{
				tcp_step = 255;
				EvLog->createEvent(EventLogger::ARELS_ERROR, 0, 0x3006);
				_status = ERROR;
			}
			break;
		default:
